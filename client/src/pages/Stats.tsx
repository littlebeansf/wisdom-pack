      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="rounded-2xl border border-border bg-muted/30 p-6 mb-6">
          ...
          <div className="h-3 rounded-full bg-muted overflow-hidden">
            <div
              ...
              style={{ width: `${completion}%`, background: "linear-gradient(90deg, #7c3aed, #4f46e5, #2563eb, #fbbf24)" }}
        ...
        <div className="space-y-3 mb-8">
          ...
          {rarities.map(rarity => {
            ...
            const rs = stats.byRarity[rarity];
            const pct = rs ? Math.round((rs.collected / rs.total) * 100) : 0;
            return (
              <div key={rarity} className="rounded-xl p-4" style={{ background: cfg.bg, border: `1.5px solid ${cfg.border}`, boxShadow: `0 0 10px ${cfg.glow}20` }} data-testid={`rarity-stat-${rarity}`}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="text-right">
                    <span className="font-bold text-white">{rs?.collected ?? 0}</span>
                    <span className="text-white/50 text-sm"> / {rs?.total ?? 0}</span>
                  </div>
                </div>
        ...
        <div className="rounded-2xl border border-border bg-muted/20 p-6 text-center">
          {completion === 0 && <><div className="text-4xl mb-3">🎴</div><p className="text-muted-foreground">Your journey begins! Open some packs to start collecting wisdom.</p></>}
          {completion > 0 && completion < 25 && <><div className="text-4xl mb-3">🌱</div><p className="text-muted-foreground">A fine start, Seeker. The path of wisdom unfolds before you.</p></>}
          {completion >= 25 && completion < 50 && <><div className="text-4xl mb-3">📚</div><p className="text-muted-foreground">Impressive! You've gathered the words of many great minds.</p></>}
          {completion >= 50 && completion < 75 && <><div className="text-4xl mb-3">⭐</div><p className="text-muted-foreground">Remarkable! Over halfway to completing your wisdom collection.</p></>}
          {completion >= 75 && completion < 100 && <><div className="text-4xl mb-3">🔥</div><p className="text-muted-foreground">Almost there! You are among the greatest collectors of wisdom.</p></>}
          {completion === 100 && <><div className="text-4xl mb-3">👑</div><p className="text-muted-foreground font-semibold">Perfect collection! You have achieved true Wisdom.</p></>}
        </div>
      </div>
    </div>
  );
